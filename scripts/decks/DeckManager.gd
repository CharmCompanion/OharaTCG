...
func assign_card_image_path(card_data: Dictionary) -> void:
        ...
        card_id = str(card_id).strip_edges().to_upper()
        ...
        # Try card_code first, then fall back to id (for reprints with different naming)
        var resolved_path := _resolve_card_image_path(set_code, card_code, zone == "search")
        if resolved_path == BASE_IMAGE_PATH and card_id != "" and card_id != card_code:
                resolved_path = _resolve_card_image_path(set_code, card_id, zone == "search")
